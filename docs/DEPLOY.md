...
3. **開発者 → Webhook → エンドポイントを追加** で、次を登録する。
   ...
   - 上4つは**決済の確定とキャンセルの判断に使う**。`completed`(決済画面を終えた)と
     `expired`(期限切れ)が主で、いまはカード決済のみを有効にしているため
     `async_payment_*` は通常発生しない。あとからコンビニ払いや銀行振込を足したときに、
     購読漏れで「支払ったのに取引が進まない」が起きないよう先に入れておく。

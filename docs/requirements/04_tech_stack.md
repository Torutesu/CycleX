...
| レイヤ | 採用技術 | 選定理由 |
|---|---|---|
| フレームワーク | **Next.js 15(App Router)+ TypeScript** | フロント/バックエンドを単一コードベースで実装。SSR で商品ページの初期表示を高速化。Vercel と組み合わせてインフラ工数ほぼゼロ |
...
| 画像処理 | Next.js Image + Supabase Storage 変換(リサイズ・サムネイル) | クライアントアップロード → サーバー側でバリデーション |
...
| テスト | Vitest(主要ビジネスロジック: 取引ステータス遷移・バリデーション)+ Playwright スモーク(主要動線 1 本) | 160h 内で費用対効果の高い範囲に限定 |

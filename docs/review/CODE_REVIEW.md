...
4. **チャージバック(不正利用の申し立て)の扱い** — 現状 `charge.dispute.created` は未購読。通知だけでも受けるか
   → 対応済み。購読して管理者へメール通知する(下記「運用ルールの決定」)
...
11. **初期メーカーマスタ** — 対応済み。ロード/クロス/MTB/e-bike の主要メーカー192件を
    `20260101000012_brand_master.sql` で投入した。過不足の確認は引き続きお願いしたい
...
| Webhook エンドポイントの登録(手順は [DEPLOY.md](../DEPLOY.md) §5。`charge.dispute.created` と `checkout.session.async_payment_*` を購読対象に含める) | 半日 |
| テストカードでの決済通し確認 | 半日 |
| Resend のドメイン認証 | 1日 |
| Google ログインの実認証 | 半日 |
| バックアップ・復旧手順の策定(手順は [BACKUP.md](../BACKUP.md) に用意済み。プランと頻度の決定が残り) | 半日 |

甲の支給待ち: 特商法表記・利用規約・プライバシーポリシー・問い合わせ窓口・
送信元アドレス・メーカーマスタの確認・管理者アカウント。
事業者情報の差し込み先は `src/lib/company.ts` に用意済み(値を入れると `/tokushoho` に出る)。
